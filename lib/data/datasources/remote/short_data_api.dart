...
class UrlDataApiImpl implements UrlDataApi {
  ...
  Future<UrlModel> updateUrl({
    ...
    try {
      logger.info('API Request: Updating URL with ID: $id');
      ...
      if (response.statusCode != 200) {
        logger.error(
          'API Error: Failed to update URL with status ${response.statusCode}',
        );
        throw ServerException(
      ...
      if (!apiResponse.isSuccess) {
        logger.error(
          'API Error: Failed to update URL with code ${apiResponse.code}',
        );
        throw ServerException(
          message: apiResponse.message ?? 'Failed to update URL',
        );
      }

      logger.info('API Success: URL updated successfully');
      return apiResponse.data;
    } on DioException catch (e) {
      logger.error('API Error: DioException while updating URL', e);
      ...
      logger.error('API Error: Unexpected error while updating URL', e);
      throw ServerException(message: 'Unexpected error: ${e.toString()}');
    }
  }
}
